...
const OnboardingOpenable: React.FC<OnboardingOpenableProps> = ({ cliStatus }) => {
  const { isDark } = useTheme();
    ...
    <button onClick={handleToggleOpened} className={cn(
      ...
      isOpened ? "h-fit pointer-events-none cursor-default p-0" : "h-[15em] hover:p-0.5 p-1 cursor-pointer"
    )}>
      <div className={cn(
        ...
        <button onClick={handleToggleOpened} className={cn(
          "absolute top-2 right-2 rounded-full hover:bg-[var(--bg-600)] p-1 transition-all pointer-events-auto",
    ...
    //   <div
    //     className={"group flex items-center justify-between px-4 py-3 bg-[var(--surface-sunken)] cursor-pointer hover:bg-[var(--surface-raised)] transition-colors rounded-sm " + (isOpened ? '' : 'border-solid border-b-2 border-[var(--surface-raised)] rounded-b-none')}
    //     onClick={handleToggleOpened}
    //   >
    //     <h2 className="text-lg font-semibold text-[var(--text-muted)] group-hover:text-[var(--text-default)]">👋 Getting Started</h2>
    //     <div className={"h-3 w-3 group-hover:bg-[var(--interactive-hover)] " + (isOpened ? 'rounded-full bg-[var(--surface-raised)]' : 'rounded-xs bg-[var(--surface-raised)]')}>
    //     </div>
    //   </div>
